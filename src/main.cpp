...
// Constants
#define SENSOR A0   // Hall senser pin
...
#define maxSpeed 300  // max motor speed
...
#define homingOffsetSteps 300 // number of steps from homing point to centre of first filter
...
#define IN2 8 //motor driver step pin
#define IN3 7
#define IN4 6 //motor driver dir pin
#define motorType 1 //1 for unipolar, 2 for motor drive board (e.g. A4988)


// SETTINGS - HALL SENSOR
#define analogSensorThreshold 50 // set the threshold below which the homing sensor is triggered
...
const String I_Strings[] = {"Arduino_Filter_Wheel_1.2", "FW3.1.6", "P", "S/N:001", "Max Speed " + String(maxSpeed), "Jitter 1", "PX Offset ", "Theshold 1", "FilterSlots " + String(numberOfFilters), "Pulse Width 4950uS"};

// Variable
int filterPos[numberOfFilters+1] = { 0, 335, 600, 1000, 1400, 1800}; //Initiaise positions Array
...
{ // locate home
  ...
  delay(1000);
  int HallValue = digitalRead(SENSOR);
  if (HallValue == LOW) {
    stepper.runToNewPosition(filterPos[currPos] - 300); // back magnet off sensor
    delay(200);
  }
...
//Serial.println(HallValue);
  ...
  delay(500); //let motor settle from decel
  digitalWrite(hallPower, LOW);
